 ...
 */
//đại diện cho một miền nhớ cụ thể nằm trong area
struct vm_rg_struct {
   int vmaid;
 ...
 *  Memory area struct
 */
//đại diện cho một phân đoạn bộ nhớ lớn (như code, stack heap)
struct vm_area_struct {
   unsigned long vm_id;
   addr_t vm_start; //giới hạn của phân đoạn
   addr_t vm_end;

   addr_t sbrk; //điểm ngăn cách giữa vùng sử dụng và chưa sử dụng
/*
 * Derived field
 * unsigned long vm_limit = vm_end - vm_start
 */
   struct mm_struct *vm_mm;
   struct vm_rg_struct *vm_freerg_list; //dslk vùng nhớ trống nằm giữa vm_start và sbrk
