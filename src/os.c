...
static void read_config(const char * path) {
	 ...
	 * Rather than a heuristic on RAM size (which fails for small RAM test cases),
	 * we read the line and check if the second token is also a number (indicating config line).
	 */
	{
		...
		if (fgets(line_buf, sizeof(line_buf), file) != NULL) {
			unsigned long ram = 0;
			unsigned long swp[PAGING_MAX_MMSWP] = {0};
			int parsed = sscanf(line_buf, "%lu %lu %lu %lu %lu", &ram, &swp[0], &swp[1], &swp[2], &swp[3]);
			if (parsed >= 2) {
				/* Memory config line is present */
				memramsz = ram;
